...
**PLAT-014-Mengenregel abgelöst (Mensch-Freigabe 2026-06-22).** Die geerbte Anzahl-Regel „nie zwei `kritisch`/`sicherheitskritisch-akut` parallel" (`KRITISCH_KLASSEN` in `check_parallel.py`) widerspricht dem Prinzip „nur Kollision, nie Anzahl" — zwei **disjunkte** `kritisch`-Specs würden sonst unnötig geblockt. Sie wird durch den strukturellen Kollisions-Schutz (Worktree-Isolation + git-non-ff-Ablehnung, s. o.) **ersetzt**. Die Code-Entfernung im freiwilligen Werkzeug `prisment-platform`-Repo (`scripts/backlog/check_parallel.py`) ist erledigt (DEFEKT-BATCH Punkt 7, AUDIT-2026-07) — das Tool bleibt als freiwilliges Disjunkt-/Hot-File-Hinweis-Werkzeug bestehen, ohne die Mengen-Regel-Prüfung. **Offener Widerspruch (Befund DEFEKT-BATCH Punkt 7):** `_Betrieb/Skripte/backlog/antritts_check.py` (Mechanismus 2 der Antritts-Selbstprüfung, PLAT-154, `status: abgeschlossen`) implementiert dieselbe Risiko-Mengen-Regel **eigenständig neu** (eigener Codeblock, `KRITISCH_KLASSEN` aus `parallel_footprint.py`) und beruft sich dabei explizit auf sie als „Reuse, Mechanismus 2" (PLAT-154_SPEC, Abschnitt F4/„Register-mechanisch urteilen") — nach dem 2026-06-22-Freigabe-Datum dieser Ablösung. Die Regel ist damit in der **aktiv laufenden** Antritts-Prüfung weiterhin scharf, obwohl dieser Absatz sie für abgelöst erklärt. Nicht in DEFEKT-BATCH Punkt 7 mitentfernt (Scope war explizit `check_parallel.py`) — Wohin-Klärung durch den Architekten offen: PLAT-154 bewusst nachträglich abweichend, oder ebenfalls Drift?
