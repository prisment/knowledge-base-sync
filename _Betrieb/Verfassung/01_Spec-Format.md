...
**Maschinelle Kollisions-Sperre, keine Anzahl-Sperre (E34-Revision, PLAT-103, Mensch-Freigabe 2026-06-22).** Im autonomen Mehr-Loop-Betrieb (3–5 parallele Loops, jeder ohne Mensch-pro-Strang) wird **kollidierende** Arbeit maschinell gestoppt — aber **nie** allein wegen der Anzahl. Der Leitstand-Claim-Ledger (`_Betrieb/Skripte/cockpit/claim.py`, eigener host-absoluter Live-Store `~/.cache/cockpit/claims.jsonl`; **seit PLAT-130 dormant** — im interaktiven Host-Architekt-Alltag kein paralleler Mehr-Loop-Dispatch; greift nur falls der autonome Orchestrator-Pfad reaktiviert wird) erkennt vor dem Bau eine Scope-Überschneidung (Seed/Spec-Gleichheit, datei-genauer Glob-Schnitt, gemeinsames Hot-File) und scheitert deterministisch (Stopp-Auslöser 5, `00_Iterationszyklus.md`); ein scope-überschreitender Build wird beim Landen geblockt (`git diff` gegen die Claim-Globs). **Disjunkte Arbeit wird nie gebremst** — das ist der Unterschied zu einer Anzahl-Wand. Erzwingend nur bei scharfem Enforcement-Schalter (Bootstrapping: Default aus). Anzahl-Steuerung bleibt Architekten-Sache (kein Loop-Zähler, keine Obergrenze).
...
```
Drei Ausgabe-Klassen: ✓ `disjunkt` / ✗ `Schnittmenge zwischen Spuren` / ✗ `Hot-File-Treffer`. Bleibt als interaktives Werkzeug nützlich; die **erzwungene** Kollisions-Sperre für autonome Loops trägt jetzt der Claim-Ledger.
...
- **Abgrenzung:** Die Halde definiert nur das „Was ist ziehbar". Die **Worker-/Loop-Mechanik** (wer zieht, wann, wie viele parallel, Lock-File etc.) ist **nicht Teil dieser Regel** und wurde als eigene Spur gebaut: der Leitstand-Claim-Ledger (PLAT-103) trägt Lock-File, Kollisions-Erkennung und Board.
