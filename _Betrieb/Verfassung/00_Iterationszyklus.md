...
- Hatte der Zyklus einen **Backlog-Seed als Auslöser**, wird in derselben Phase-9-Bewegung sein `status:` auf `abgeschlossen` gezogen. Ein Seed ohne Folge-Glieder wandert danach ins Archiv des Zyklus. Ein Seed, der Vorgänger einer noch offenen `abhaengig_von`-Kette ist, bleibt als abgeschlossener Lese-Anker in `seeds/` stehen (Worker-Vorgänger-Prüfung, siehe `01_Spec-Format.md` Abschnitt „Autonome Halde"); er wandert erst beim Ketten-Kehraus — wenn das letzte Glied der Kette abschließt, gehen alle abgeschlossenen Seeds der Kette gemeinsam ins Archiv. (Teil-abgearbeitet → bleibt im Backlog mit Hinweis-Block + Querverweis auf die Spec.)
