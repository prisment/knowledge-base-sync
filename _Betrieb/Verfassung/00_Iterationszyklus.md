...
Der verbindliche Arbeitsablauf für jeden Zyklus. Alle drei Akteure (Mensch, **Architekt** [der beratende Claude-Agent am Host — er sondiert, erarbeitet und empfiehlt; **jedes Wohin gehört dem Menschen** — seit PLAT-130 keine getrennte Windows-Maschine mehr], **Claude Code** [die ausführende Hand am System, als Worker-Subagent oder Haupt-Session]) kennen ihn.

**Rollen-Abgleich (CoS-Reform 2026-07-20, nachgezogen 2026-07-30):** Die heutige Ebenen-Topologie lautet: **Ebene 0 = Chief of Staff** (die Standard-Hauptsession, Charta `.claude/agents/chief-of-staff.md`) · **Ebene 1 = Zyklus-Architekt** (`architekt`-Subagent, einer pro Bau-Zyklus, Skill `zyklus-master`) · **Endstufe = worker/pruefer/evaluator/erhebung/sondierung/recherche**. Wo diese Verfassung „Architekt" oder „Claude Code" sagt, ist die Rolle nach dieser Karte gemeint (Ebene 0 oder 1 als beratend-orchestrierende Hand, Endstufe als ausführende) — Akte, Stufen und Tore gelten unverändert für jede Ebene, die sie fährt.
...
**Kalter Evaluator (Pflicht-Pass für Sprung und Spur).** Bevor eine Spec dem Menschen zur Freigabe vorgelegt (Spur) oder autonom gestartet (Sprung) wird, läuft ein kalter Evaluator-Aufruf (`evaluator`-Subagent, eigener frischer Kontext, Modell Opus / effort high — der frühere headless `claude -p`-Weg ist abgelöst, PLAT-130; Verfahren: Skill `evaluator-pass`). Der schreibende Agent hat Einwände-Behandlungspflicht: jeder Evaluator-Einwand wird in der Spec-Sektion „Einwände & Behandlung" entweder eingearbeitet oder explizit widerlegt. Der Evaluator hat kein Veto (beratend) — mit EINER Ausnahme: beim Verdikt `RICHTUNGSWECHSEL_PFLICHT` ist die Wiedereinreichung des **gleichen Ansatzes** gesperrt (entweder den alternativen Pfad des Evaluators übernehmen oder eskalieren — Sackgassen-Protokoll, `prompt_eval1.txt`); und eine Spec mit unbehandelten Einwänden ist nicht freigabefähig (Korridor-Wand). Selbst-Evaluation ersetzt den Pass nie (Generator ≠ Evaluator).
...
Ebene 2  Endstufe: erhebung/sondierung/recherche/worker/evaluator/pruefer
         — worker (trägt das Agent-Tool) darf read-only Lese-Helfer
           (erhebung/recherche) für Kontext-Hygiene spawnen (Lese-Helfer-
           Regel, Ruling 2026-07-19); alle anderen spawnen NIE (mechanisch:
           kein Agent-Tool in ihrer tools-Liste)
```

- **Erlaubnis, nicht Fähigkeit.** Technisch erben alle Agent-Typen ohne `tools:`-Feld das Agent-Tool — die Endstufen-Regel ist Prompt-Korsett + Doktrin, keine Harness-Sperre. `architekt` ist der einzige Typ mit **vollem** Spawn-Recht auf alle Endstufen; `worker` trägt daneben nur das begrenzte **Lese-Helfer**-Spawn-Recht (`erhebung`/`recherche`, read-only — niemals bauen, urteilen oder kalt prüfen unterhalb der eigenen Ebene; Ruling 2026-07-19). Ein `architekt` spawnt nie selbst einen `architekt` (effektive Maximal-Tiefe: Master → architekt → worker → Lese-Helfer). **Endstufe ist eine Rollen-Eigenschaft, keine Ebenen-Nummer** — der klassische Ebene-0-Betrieb (Session dispatcht Endstufen direkt) bleibt vollständig gültig.
...
- **Unverändert gelten:** Wohin=Mensch an jeder Zyklus-Grenze (Warte-Tor — kein Folge-Zyklus ohne Mensch-Wort), Rolle B (Laufzeit-Tests nur worker/pruefer, nie `architekt` selbst — Herkunft: früher mechanisch mit-erzwungen via einem eigenen Confinement-Hook, der mit dem Produkt ausgegliedert wurde), irreversible Außen-Aktionen sind Mensch-Hand, WIP-Regel (Herkunft PLAT-165, Spec archiviert), alle Stop-Tore.
...
**Tageszeit ist auch kein Stopp-Auslöser** (PLAT-200, Korbinian-Ruling 25.08.2026): es gibt keinen sechsten Auslöser „Abend/Nacht/Feierabend". Tages-Rituale (Lage-Push) sind Meldungen im Vorbeigehen, nie Arbeits-Enden. Sitzungs-/Token-Erschöpfung bleibt ein ehrlicher physischer Stopp — er wird gemeldet und hinterlässt den Wieder-Anlauf-Pfad (Wecker-Kette + Wieder-Anlauf von Platte), er wird nur nie als Feierabend gerahmt.
