...
## E52 — Unbeaufsichtigter Worker für autonom_ziehbar-Seeds gebaut, Verfassungs-Variante δ (Ketten-Kehraus) gewählt (PLAT-035, 2026-05-27)

**Warum:** PLAT-033/034 hatten die Halde-Mechanik als Konzept etabliert (`autonom_ziehbar`-Feld als abgeleiteter Zustand, freigegebene-sicher-Spec-Halde), aber keinen Mechanismus, der die Halde tatsächlich autonom abarbeitet. Der Mensch zog manuell aus der Halde, was den Sinn der Autonomie-Mechanik halbierte. PLAT-035 baut den Worker, der die Halde unbeaufsichtigt abarbeitet — in Koexistenz mit manuell parallel gefahrenen Spuren, mit drei-facher Vorgänger-Prüfung gegen Race-Bedingungen, mit fail-safe Blocker-Erkennung (Unsicherheit zählt als Blocker), mit Mail-Eskalation bei Blocker / kritischer Seed-Berührung, mit persistentem Frozen-Set.

**Verfassungs-Variante δ statt α gewählt (Architekten-Entscheidung):** Beim Phase-9-Abschluss eines Seeds, der Vorgänger einer noch offenen `abhaengig_von`-Kette ist, bleibt der Seed als abgeschlossener Lese-Anker in `_Betrieb/Backlog/seeds/` stehen. Erst beim Abschluss des letzten Glieds der Kette wandern alle abgeschlossenen Seeds der Kette gemeinsam in einen Archiv-Ordner (`<Bereich>/Archiv/<spec_id-des-letzten-Glieds>/`). Alternative α (Seed bleibt dauerhaft in `seeds/`) wäre Dauerhaft-Wachstum gewesen; γ (Worker liest Spec-Archive zusätzlich) hätte zwei Lese-Pfade gehabt. δ ist die einzige Variante, die SSOT-treu (eine Lese-Quelle in `seeds/`) und gleichzeitig anti-wachstum ist.
...
**Pilot-Ergebnis:** Drei autonome Phase-9-Merges auf main: PLAT-036 (`8709c73`, next-spec-id-Patch), PLAT-037 (`7712ccd`, Notausgangs-Doku), PLAT-038 (`bb71494`, DNS-Hairpin-Diagnose). Alle drei inhaltlich sauber, Akzeptanzkriterien grün, ehrliche Abweichungs-Dokumentation, Folge-Befunde sauber abgegrenzt, keine stillen Scope-Erweiterungen. Die Worker-Kern-Mechanik (Halde-Auswahl, Worktree-Anlage, Subprocess-Übergabe, Frozen-Set, Mail, Ketten-Kehraus auf Seeds) hat funktioniert.

**Gleichzeitig: 100% False-Positive-Rate des Worker-Detektors.** Alle drei erfolgreichen Läufe wurden vom Worker als Blocker klassifiziert. Damit ist der Detektor strukturell unbrauchbar — er hat null Aussagekraft, weil eine Erkennung, die in Richtung Falsch-positiv 100% liegt, auch in Richtung Falsch-negativ unscharf sein kann (gefährlicher Zwilling). Genau die Lage aus E52 Trigger (b): „bei falschen Alarmen Pilot-Pause, nicht Marker-Liste schrumpfen". Pilot wurde sauber via `enabled: false`-Edit beendet, kein Subprocess-Killen.

**Drei strukturelle Befunde (Fix-Specs folgen):**
- A — Marker-Filter ist Prosa-Grep über Sub-Claude-Output. Beiläufige Erwähnungen von „stopp-auslöser" o.ä. triggern Blocker. Lösung-Richtung: strukturiertes Status-Signal (`WORKER_STATUS: clean|blocked:<grund>`) als letzte Pflicht-Zeile, statt Fließtext-Grep. Wird als PLAT-039 (sprung, kritisch, Architekten-Hand) geführt.
- B — `_verify_phase9` schaut nur in Worktree-Pfad. Nach Phase-9-Merge räumt Sub-Claude den Worktree weg → Verify findet keine ABSCHLUSS-Datei mehr → False-Positive. Lösung: Verifikation am Merge-Commit auf main statt am Worktree-Dateipfad.
...
**Warum:** Pilot 1 hatte 100% False-Positive-Rate auf drei erfolgreichen Phase-9-Merges (E55). Drei strukturelle Befunde identifiziert (A: Prosa-Grep-Marker, B: Verify am Worktree-Pfad, C: Tool archiviert nur Seeds). Beide Fix-Specs sind durch:

- **PLAT-039** (Worker-Status-Signal): Sub-Claude setzt jetzt pflicht-mäßig eine letzte Zeile `WORKER_STATUS: clean | blocked:<grund>`. Worker klassifiziert ausschließlich darüber; Prosa-Grep ist raus. Fail-safe bleibt: fehlende Zeile → Blocker.
- **PLAT-040** (Verifikation am Merge-Commit + vollständiger Kehraus + Z.159-Schärfung): Verfassungs-Bullet „Vollständiger Zyklus-Satz, Tool-Zuständigkeit" eingearbeitet (Verfassung 01 + 00 Z.159). `_verify_phase9` prüft `[<spec_id>]`-Commit auf origin/main + Spec/Seed-Status (Arbeitsgedächtnis oder Archiv) statt Worktree-Glob. `phase9_seed_archive.py` verschiebt vollständigen Zyklus-Satz (Spec, Abschluss, Machbarkeit, Fakten, Deviation-Logs — soweit vorhanden) gemeinsam ins Archiv. Sub-Claude-Template enthält ausdrückliches Verbot manueller Archiv-Moves.

**Parallel-Lauf-Pannenfall:** Während meiner Implementierung lief ein zweiter Agent unbemerkt parallel an PLAT-040. Architekt hat den Parallel-Agent beendet, sein Stash mit PLAT-026-Backup-flock-Integration wurde gerettet (Commit `d16edb6` platform), Bündel-3-Code-Anteil war redundant zu meinem (semantisch identisch). Wichtiger Methodik-Befund für die Zukunft: bei mehrstündigen Specs Parallel-Lauf-Disziplin oder explizite Hand-Off-Marker nötig.
