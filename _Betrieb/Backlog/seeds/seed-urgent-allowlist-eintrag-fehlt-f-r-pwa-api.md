---
typ: backlog_seed
titel: "Allowlist-Eintrag fehlt für pwa-api"
...
trigger: coverage-allowlist-fehlt-pwa-api
...
**Trigger-Key (Dedup-Anker):** `coverage-allowlist-fehlt-pwa-api` — solange dieser Seed
`status: offen` trägt, bumpt jeder neue Befund nur `gesehen_am` und
`## Letzter Befund`, kein zweiter Seed wird angelegt.

## Letzter Befund
_(2026-05-27)_ Service pwa-api ist Klasse A/B in Registry, aber /etc/claude/nightly-allowlist.conf hat keine build-Stelle. registry-apply.sh erneut laufen lassen + sudo-Install des Allowlist-Fragments.
