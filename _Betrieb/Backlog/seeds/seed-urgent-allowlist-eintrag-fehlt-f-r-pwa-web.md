---
typ: backlog_seed
titel: "Allowlist-Eintrag fehlt für pwa-web"
...
trigger: coverage-allowlist-fehlt-pwa-web
...
**Quelle:** check-service-coverage @ 2026-05-27T13:44:30+02:00

**Trigger-Key (Dedup-Anker):** `coverage-allowlist-fehlt-pwa-web` — solange dieser Seed
`status: offen` trägt, bumpt jeder neue Befund nur `gesehen_am` und
`## Letzter Befund`, kein zweiter Seed wird angelegt.

## Letzter Befund
_(2026-05-27)_ Service pwa-web ist Klasse A/B in Registry, aber /etc/claude/nightly-allowlist.conf hat keine build-Stelle. registry-apply.sh erneut laufen lassen + sudo-Install des Allowlist-Fragments.
