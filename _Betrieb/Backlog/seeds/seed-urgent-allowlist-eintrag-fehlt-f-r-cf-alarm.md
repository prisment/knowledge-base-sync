---
typ: backlog_seed
titel: "Allowlist-Eintrag fehlt für cf-alarm"
...
trigger: coverage-allowlist-fehlt-cf-alarm
...
**Trigger-Key (Dedup-Anker):** `coverage-allowlist-fehlt-cf-alarm` — solange dieser Seed
`status: offen` trägt, bumpt jeder neue Befund nur `gesehen_am` und
`## Letzter Befund`, kein zweiter Seed wird angelegt.

## Letzter Befund
_(2026-05-27)_ Service cf-alarm ist Klasse A/B in Registry, aber /etc/claude/nightly-allowlist.conf hat keine build-Stelle. registry-apply.sh erneut laufen lassen + sudo-Install des Allowlist-Fragments.
