...
### 6. `beruehrt:`-Feld + Hot-Files — Datenbasis für Parallelität

Der Autonomie-Zyklus (Phase 2, Worktrees) braucht eine maschinell prüfbare
Antwort auf „welche zwei Seeds sind parallelisierbar?". Parallelisierbar ⟺ ihre
berührten Dateien/Ordner schneiden sich nicht. Dafür:

**Neues Front-Matter-Feld `beruehrt:`** — Liste der Pfade/Ordner, die ein Seed
bei Umsetzung anfasst. **Ordner-Ebene genügt** (`src/features/`,
`src/landingpage/`) — gröber, robuster, weniger Pflege als datei-genau.
Beispiel:
```yaml
beruehrt: ["src/agents/", "src/lib/datapoints/"]
```
Wird beratend vorgeschlagen (Claude Code kennt den Code), Mensch revidiert. Bei
Seeds, deren Footprint noch unklar ist (Details erst in der Spec), bleibt das
Feld leer/grob und wird beim Spec-Start präzisiert.

**Hot-Files (strang-übergreifend, nur seriell):** Bestimmte Dateien werden von
fast jedem Strang angefasst und sind die typische Konflikt-Quelle —
`package.json`, Lockfiles, zentrale Config, DB-Migrationen. Diese werden im
Framework explizit als **„Hot File — nur seriell anfassen, nie in zwei parallelen
Strängen"** markiert. Sie brechen sonst die schöne Disjunktheits-Theorie in der
Praxis. Pflege als kurze Liste (eigene Sektion in der Übersicht oder Verfassungs-
Vermerk — in der Spec entscheiden).

**Das Übersichts-Skript wertet `beruehrt:` mit aus:** Es markiert Seed-Paare als
„parallel-sicher", wenn die Footprints disjunkt sind UND keine gemeinsame Hot-File
berührt wird. Aus der Prio-Übersicht wird damit zugleich eine
**Parallelisierungs-Karte** — sie beantwortet nicht nur „was zuerst?", sondern
auch „was gleichzeitig?".

### 7. `kritisch`-Flag pro Bundle (Voraussetzung für Korridor-Autonomie)

Der Autonomie-Zyklus braucht, dass die Spec vorne pro Bundle markiert, ob es
kritisch ist (autonom durchlaufen vs. synchroner Stopp). Die vollständige
Definition (feste Kritikalitäts-Liste + zwei Testfragen + Nur-nach-oben-Regel)
lebt im Autonomie-Seed und wird dort in `01_Spec-Format` verankert. Hier nur der
Querverweis: Das `kritisch`-Flag ist ein weiteres maschinell auswertbares
Front-Matter-/Bundle-Feld, das dieselbe „KI schlägt vor, Mensch revidiert,
nur-nach-oben"-Mechanik nutzt wie `stufe`, `klasse`, `zugkraft`. Konsistenz der
Feld-Logik wahren; nicht doppelt definieren.
...
- `ROADMAP_lebende-bereichs-doku` Schritt 8: dieselbe Pflege-Mechanik
  ...
  erfinden.
- **`seed-autonomie-korridor-parallele-sessions` (NEU, Spur):** Beidseitige
  Abhängigkeit. Dieser Seed liefert `beruehrt:` + Hot-Files +
  Parallelisierungs-Karte, die der Autonomie-Seed (Phase 2, Worktrees)
  voraussetzt. Reihenfolge-Empfehlung: dieser Priorisierungs-Seed ZUERST (er
  schafft die Datenbasis), Autonomie-Phase-2 danach. Autonomie-Phase-1
  (Korridor-Modell) ist unabhängig und kann parallel/vorher laufen.
...
- **Fünfte Klasse `prozess`/`framework`?** Seeds, die das Framework selbst
  betreffen (dieser, `seed-autonomie-korridor`, `seed-skills-evaluieren`,
  `seed-feature-flag-zustand`), passen in keine der vier Bedarfs-Klassen
  (security/feature/qualitaet/wartung) sauber. Entweder fünfte Klasse `prozess`
  einführen ODER unter `wartung` fassen. Echte Designentscheidung, kein Detail —
  beim Spec-Start klären, weil mehrere Seeds (inkl. dieser) davon abhängen.
- **`beruehrt:`-Granularität final:** Ordner-Ebene (Empfehlung) bestätigen oder
  datei-genau? Trade-off Pflegeaufwand vs. Präzision der Parallel-Erkennung.
- **Hot-File-Liste:** wo gepflegt (Übersichts-Sektion vs. Verfassung), und wie
  hält sie sich aktuell (Phase-9-Pflicht-Tor-Muster)?
- **Parallelisierungs-Karte:** eigene SVG/Sektion in der Übersicht oder in den
  bestehenden Abhängigkeits-Graph integriert (z. B. parallel-sichere Paare grün
  verbunden, Hot-File-Kollisionen rot)?
